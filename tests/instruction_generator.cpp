...
TEST(instruction_generator_tests, constants)
{
  ...
  for (uint8_t i = 0; i < num_const_tests; i++) {
    auto value = libutil::random::generate_random_c<uint8_t>().get_range(
  ...
  //
  for (uint8_t i = 0; i < num_const_tests; i++) {
    auto value = libutil::random::generate_random_c<uint16_t>().get_range(
  ...
  //
  for (uint8_t i = 0; i < num_const_tests; i++) {
    auto value = libutil::random::generate_random_c<uint32_t>().get_range(
  ...
  //
  for (uint8_t i = 0; i < num_const_tests; i++) {
    auto value = libutil::random::generate_random_c<uint64_t>().get_range(
  ...
  //
  for (uint8_t i = 0; i < num_const_tests; i++) {
    auto value = libutil::random::generate_random_c<int8_t>().get_range(
  ...
  //
  for (uint8_t i = 0; i < num_const_tests; i++) {
    auto value = libutil::random::generate_random_c<int16_t>().get_range(
  ...
  //
  for (uint8_t i = 0; i < num_const_tests; i++) {
    auto value = libutil::random::generate_random_c<int32_t>().get_range(
  ...
  //
  for (uint8_t i = 0; i < num_const_tests; i++) {
    auto value = libutil::random::generate_random_c<int64_t>().get_range(
  ...
  //
  for (uint8_t i = 0; i < num_const_tests; i++) {
    std::string s = libutil::random::random_string_c().generate_string(
        libutil::random::generate_random_c<uint8_t>().get_range(10, 20));
...
TEST(instruction_generator_tests, registers)
{
  ...
  for (auto i = 0; i < 10; i++) {
    auto value = gen.get_register_value(
        libutil::random::random_string_c().generate_string(3));
    CHECK_TRUE(value == std::nullopt);
  }
}
...
TEST(instruction_generator_tests, instruction_branch)
{
  ...
  for (auto &ins : {libskiff::bytecode::instructions::BLT,
    ...
    auto lhs_register =
        known_good_registers[libutil::random::generate_random_c<uint8_t>()
                                 .get_range(0,
    ...
    auto address = libutil::random::generate_random_c<uint64_t>().get_range(
        std::numeric_limits<uint64_t>::min(),
        std::numeric_limits<uint64_t>::max());
